...
import { apAging, AGING_LABELS, type AgingBucket } from "@/lib/reports";
...
export default async function ApAgingPage() {
  const report = await apAging();

  return (
    <AppShell title="A/P aging" subtitle="Open vendor bills by days past due">
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
        ...
        <div className="bg-surface border border-amber-500/20 rounded-lg p-3">
          <div className="text-[10px] uppercase tracking-wider text-amber-400/80 font-body">Total AP</div>
        ...
        <table className="w-full text-sm min-w-[640px]">
          ...
          <tbody className="font-body text-zinc-200">
            {report.rows.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-8 text-center text-xs text-zinc-500">No open payables.</td></tr>
            ) : (
              report.rows.map((r) => (
                <tr key={r.id} className="border-t border-white/5 hover:bg-white/5">
                  <td className="px-4 py-2.5">
                    <Link href={`/accounting/bills/${r.id}`} className="font-mono text-xs text-white hover:text-amber-300">{r.number}</Link>
                  </td>
