    ...
    <nav className="border-b border-white/5 px-6" ref={navRef}>
      <ul className="flex gap-1 overflow-visible">
        {NAV.map((group, idx) => {
          ...
          const baseClasses =
            "inline-block px-3 py-2.5 text-xs font-body border-b-2 transition-colors whitespace-nowrap";
          ...
          return (
            <li key={group.label} className="relative">
              <button
                ...
                className={`${baseClasses} ${stateClasses} flex items-center gap-1 cursor-pointer`}
              >
                {group.label}
                <svg
              ...
              {isOpen ? (
                <div
                  role="menu"
                  className="absolute left-0 top-full mt-0 z-50 min-w-[180px] bg-zinc-950 border border-white/10 rounded-md shadow-lg py-1"
                >
